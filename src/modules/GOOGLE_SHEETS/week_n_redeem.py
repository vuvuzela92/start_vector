...
WITH week_rep AS ( -- Данные еженедельного отчета реализации
	...
	SELECT 
		...
		(f.return_pay+reimbursement_of_transp_costs+
			reimbursement_for_delivery_and_return_of_goods_to_pvz+penalties+
			other_deductions+retentions_in_favor_of_third_parties+COALESCE(amount_withheld_to_org, 0)) AS "Итого_расходы",
		(
			w.total_sum+damages_comp_other+r.sum_rub_with_vat+
			(CASE 
				WHEN w.award < 0
					THEN w.award*-1
				ELSE w.award -- Доходы минус расходы
			END)
		)
		-
		(
		f.return_pay+reimbursement_of_transp_costs+
			reimbursement_for_delivery_and_return_of_goods_to_pvz+penalties+
			other_deductions+retentions_in_favor_of_third_parties+COALESCE(amount_withheld_to_org, 0)
			) AS "К_перечислению_по_отчетам"
	FROM week_rep w
