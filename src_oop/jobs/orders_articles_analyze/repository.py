...
class ArticleAnalyzeRepository:
    ...
    def get_general_stat(self, days_ago: int, days_to: int):
        ...
        # Оптимизируем UNION. Сначала делаем быстрые выборки UNION ALL, 
        # а потом ОДИН раз группируем их, получая уникальный список (это в разы быстрее!)
        query = text(f"""
            WITH raw_base AS (
            ...
            base AS (
                SELECT article_id, "date"
                FROM raw_base
                GROUP BY article_id, "date" -- Одиночная группировка вместо трех UNION
            )
            SELECT 
                b.date, 
