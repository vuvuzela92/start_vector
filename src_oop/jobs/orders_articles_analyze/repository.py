...
class ArticleAnalyzeRepository:
    ...
    def get_general_stat(self):
            """Получение общих статистических данных для Артикульного анализа"""
            query = text(f"""-- == version 2.0 ==
                WITH base AS (
                    ...
                    WHERE DATE(date_from) BETWEEN CURRENT_DATE - INTERVAL '{self.days_ago} days' AND CURRENT_DATE - INTERVAL '{self.days_to} days'
                    UNION 
                ...
                SELECT 
                    ...
                    -- Прибыль по заказам (Индивидуальные условия)
                    ...
                    -- Прибыль по продажам (Индивидуальные условия)
                    ROUND(
                    ...
                    COALESCE(fin.sales_revenue_rep, 0) - COALESCE(fin.wb_commission_rep, 0) - COALESCE(fin.logistics, 0) - (cp.purchase_price*COALESCE(fin.sales_count_rep, 0)) AS sales_profit_cond_rep, 
                ...
                LEFT JOIN (
                    ...
                    FROM sales s 
                    WHERE is_realization IS TRUE AND s.date BETWEEN CURRENT_DATE - INTERVAL '{self.days_ago} days' AND CURRENT_DATE - INTERVAL '{self.days_to} days'
                ...
                LEFT JOIN (
                    ...
                    WHERE DATE(fin.date_from) BETWEEN CURRENT_DATE - INTERVAL '{self.days_ago} days' AND CURRENT_DATE - INTERVAL '{self.days_to} days'
