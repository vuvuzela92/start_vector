...
def prepare_purchase_price_updates(
    ...
    Логика намеренно повторяет смысл старого скрипта:
    1. нормализуем ключи и цену из БД;
    2. при необходимости применяем legacy-округление;
    ...
    `round_price` — нужно ли применять legacy-совместимое округление новой цены.
