...
PURCHASE_PRICE_UPDATE_QUERY = text(
    ...
    WITH latest_purchase_price AS (
        ...
        WHERE is_valid = TRUE
          ...
          AND quantity != 0
          -- В расчёте закупочной цены участвуют только документы прихода.
          -- В исторических данных префикс `ВВЦБ` может быть записан латинскими
          -- буквами `BBЦБ`, поэтому явно поддерживаем оба варианта написания.
          AND (
              document_number LIKE 'BBЦБ%'
              OR document_number LIKE 'ВВЦБ%'
              OR document_number LIKE 'СТЦБ%'
          )
