...
class LogisticVedReverseUpdater:
    ...
    def _validate_headers(
        ...
        логистами. Номер трака и номер ТС нужны только в ``ОТЧЁТ_2.0``:
        по ним сопоставляется фактическая приемка из БД. Ранняя
    ...
    def _build_acceptance_key(
        ...
        """Собирает нормализованный ключ автоприемки из трех бизнес-полей.

        Бизнес-логика:
        приемка подтверждается по совпадению номера трака, номера ТС и wild.
        Количество не входит в ключ, потому что оно проверяется отдельно и при
        расхождении переводит строку в статус ``расхождения при приемке``.
        """
        return (
