...
class LogisticVedService:
    """Оркестрация получения и выгрузки данных logistic_ved."""

    repository: LogisticVedRepository
    sheets_client: LogisticVedGoogleSheetsClient

    def run(self) -> None:
        logger.info("Старт задачи logistic_ved.")
        ...
        logger.info(
            "Задача logistic_ved завершена успешно: rows=%s",
            len(prepared_dataframe.index),
        )

    def _prepare_for_google_sheets(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        logger.info(
            "Подготовка DataFrame logistic_ved к выгрузке: rows=%s columns=%s",
            len(dataframe.index),
            list(dataframe.columns),
        )

        renamed_dataframe = dataframe.rename(columns=LOGISTIC_VED_COLUMN_MAPPING)
        ordered_dataframe = renamed_dataframe.reindex(columns=LOGISTIC_VED_COLUMNS)

        if ordered_dataframe.empty:
            logger.warning(
                "После подготовки logistic_ved DataFrame пуст. В Google Sheets будут записаны только заголовки."
            )
