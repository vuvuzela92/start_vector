...
class VedBalanceAnalyticsService:
    ...
    Сервис:
    - читает лист `Логистика ВЭД 2026 / ОТЧЁТ_2.0`;
    ...
    # Базовые поля VED приводим к общей структуре `Аналитика_платежей`.
    # В VED колонка `Поставщик` хранит не товарного поставщика, а контрагента
    # текущего этапа: по умолчанию это перевозчик, а для брокерского этапа
    # значение позже отдельно перезаписывается из `Декларант`.
    BASE_COLUMNS_MAP = {
        "Логист": "Закупщик",
        ...
        "Модель": "Модель",
        "КОМПАНИЯ": "КОМПАНИЯ",
        "Кол-во к заказу": "Кол-во к заказу",
    ...
    def load_source_data(self) -> pd.DataFrame:
        ...
        В листе заголовки находятся на четвертой строке, поэтому при чтении
        ...
        """
        # В листе ВЭД заголовки находятся на четвертой строке, а данные идут ниже.
